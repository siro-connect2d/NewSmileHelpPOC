...
//  NewSmileHelpPOC
//
//  Created by Siroson on 2021-07-27.
//
...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let dummyNavi = UINavigationController(rootViewController: dummyVC)
        dummyNavi.tabBarItem = UITabBarItem(tabBarSystemItem: .history, tag: 1)
        
        //tabbar controller
        let tabbarController = UITabBarController()
        tabbarController.delegate = self
        tabbarController.viewControllers = [homeNavi, dummyNavi]
